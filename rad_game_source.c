...
#pragma region Modes and Levels
static struct
{
    enum ERadGameModes cur_mode;
    int score;		//!< only used in show_score mode
    int player;		//!< only used in ready modes
} rad_game_mode;
...
static void RadMovingObject_render(struct RadMovingObject* mo, int color, ws2811_t* ledstrip)
{
    if (mo->moving_dir == 0)
    {
...
static int Oscillators_render(ws2811_t* ledstrip, double unhide_pattern)
{
...
void OscPlayer_move(int player_index, signed char dir)
{
    if (osc_players.pos[player_index].moving_dir == 0 && 
        osc_players.pos[player_index].position > oscillators.end_zone_width &&
        osc_players.pos[player_index].position < rad_game_source.basic_source.n_leds - oscillators.end_zone_width)
    {
...
*/
void OscPlayer_strike(int player_index, enum ERAD_COLOURS colour)
{
    (void)colour;
...
void OscPlayers_init()
{
    Player_hit_color = OscPlayer_strike;
    Player_move = OscPlayer_move;
...
void Player_move_ddr(int player_index, signed char dir)
{
    (void)player_index;
    (void)dir;
}
...
int RGM_DDR_Ready_update_leds(int frame, ws2811_t* ledstrip)
{
    return 1;
}
...
int(*get_update_function())(int, ws2811_t*)
{
    ...
    case RGM_DDR_Ready:
        return RGM_DDR_Ready_update_leds;
    case RGM_N_MODES:
        exit(-1);
    }
    exit(-2);
